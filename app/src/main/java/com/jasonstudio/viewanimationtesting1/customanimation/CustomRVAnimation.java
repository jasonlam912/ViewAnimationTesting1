package com.jasonstudio.viewanimationtesting1.customanimation;
...
public class CustomRVAnimation extends DefaultItemAnimator {
    ...
    public boolean animateChange(ViewHolder oldHolder, ViewHolder newHolder,
        ...
        List<Integer> list = Arrays.asList(fromX, fromY, toX, toY);
        //Log.d("animateChange", list.stream().map(i -> i.toString()).collect(Collectors.joining(", ")));
        if(oldHolder == newHolder){
            Log.d("animatChang","old and new holder are same");
        }
        if (oldHolder == newHolder) {
            // Don't know how to run change animations when the same view holder is re-used.
            // run a move animation to handle position changes.
